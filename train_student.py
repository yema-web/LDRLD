...
from distiller_zoo import DistillKL, HintLoss, Attention, Similarity, Correlation, VIDLoss, RKDLoss, LDRLD
from distiller_zoo import PKT, ABLoss, FactorTransfer, KDSVD, FSP, NSTLoss,WSLDLoss,DKD,build_review_kd
...
def parse_option():
    ...
    # distillation
    parser.add_argument('--distill', type=str, default='kd', choices=['kd','ldrld', 'dkd','hint', 'attention', 'similarity',
                                                                      'correlation', 'vid', 'crd', 'kdsvd', 'fsp',
                                                                      'rkd', 'pkt', 'abound', 'factor','ReviewKD', 'nst','wsld'])
    ...
    opt.model_name = 'S:{}_T:{}_{}_{}_r:{}_a:{}_b:{}_b:{}_{}'.format(opt.model_s, opt.model_t, opt.dataset, opt.distill,
                                                                opt.gamma, opt.alpha, opt.beta,  opt.k,opt.trial)
...
def main():
    ...
    if opt.distill == 'kd':
        ...
        criterion_kd = LDRLD(opt.alpha, opt.beta, k=opt.k, temperature=opt.kd_T, ce_loss_weight=opt.gamma)
    ...
    elif opt.distill == 'wsld':
        criterion_kd = WSLDLoss()
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        if epoch >10:
            print('epoch {}, our_total time {:.2f}'.format(epoch, total_time/epoch))
